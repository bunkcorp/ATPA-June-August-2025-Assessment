#!/usr/bin/env python3
"""
Realistic Oversampling Analysis - No Data Leakage
ATPA Assessment - June to August 2025

This script implements oversampling techniques using features that don't
directly predict the target variable to avoid data leakage.
"""
...
def load_and_prepare_data():
    """Load and prepare the dataset without data leakage"""
    ...
    try:
        ...
        print("Error: incidents_with_arrests.csv not found.")
        return None, None, None
    
    # Use features that don't directly predict the target
    # Avoid num_arrests as it directly determines MULTIPLE_ARRESTS
    feature_cols = [
...
def implement_oversampling_techniques(X_train, y_train, X_test, y_test):
    ...
    print(f"Training set after ADASYN: {len(X_train_adasyn)} samples")
    print(f"Class distribution after ADASYN: {np.bincount(y_train_adasyn)}")
    ...
    print(f"Training set after SMOTE+ENN: {len(X_train_smoteenn)} samples")
    print(f"Class distribution after SMOTE+ENN: {np.bincount(y_train_smoteenn)}")
...
def save_results_to_json(results):
    ...
    with open('realistic_oversampling_results.json', 'w') as f:
        json.dump(results_json, f, indent=2)
    
    print("\nResults saved to 'realistic_oversampling_results.json'")

def main():
    """Main execution function"""
    
    print("Realistic Oversampling Analysis - No Data Leakage")
    ...
    baseline_sensitivity = results['Baseline']['sensitivity']
    best_sensitivity = results[best_sensitivity_model]['sensitivity']
    sensitivity_improvement = ((best_sensitivity - baseline_sensitivity) / baseline_sensitivity) * 100
    
    print(f"\nSensitivity Improvement: {sensitivity_improvement:.1f}%")
    print(f"Baseline Sensitivity: {baseline_sensitivity:.3f}")
    ...
    if sensitivity_improvement > 10:
        print(f"✅ YES - Oversampling improves sensitivity by {sensitivity_improvement:.1f}%")
        print(f"   Recommended technique: {best_sensitivity_model}")
    else:
        print("❌ NO - Oversampling provides minimal improvement")
        print("   Consider other approaches like feature engineering")
