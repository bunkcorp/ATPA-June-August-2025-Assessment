...
def create_shap_visualizations(explainer, shap_values, X_selected, y_selected, feature_cols):
    ...
    shap.summary_plot(shap_values, X_selected, feature_names=feature_cols, show=False)
    plt.title('SHAP Summary Plot for Selected Incidents')
    plt.tight_layout()
    plt.savefig('task4_shap_summary.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # Create individual plots for each selected incident
    fig, axes = plt.subplots(2, 3, figsize=(20, 12))
    axes = axes.flatten()
    
    for i, (idx, (_, incident), actual) in enumerate(zip(range(len(X_selected)), X_selected.iterrows(), y_selected)):
        ax = axes[i]
        
        # Create waterfall plot for individual incident
        shap.waterfall_plot(
            shap.Explanation(
        ...
        ax.set_title(f'Incident {i+1}: {"Arrested" if actual == 1 else "Not Arrested"}')
...
2. **Most Influential Features**:
   - Sex code has the strongest impact on predictions
