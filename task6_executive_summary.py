...
def load_all_results():
    ...
    try:
        # Load Task 3 results (GLM performance)
        glm_results = pd.read_csv('../Task3_Modeling/task3_model_report.txt', sep='\n', header=None)
        results['task3'] = {'glm_report': glm_results}
        print("Task 3: GLM model results loaded")
    except FileNotFoundError:
        print("Task 3 results not found")
    
    try:
        # Load Task 4 results (Random Forest performance)
        rf_results = pd.read_csv('../Task4_RandomForest/task4_report.txt', sep='\n', header=None)
        results['task4'] = {'rf_report': rf_results}
        print("Task 4: Random Forest results loaded")
    except FileNotFoundError:
        print("Task 4 results not found")
